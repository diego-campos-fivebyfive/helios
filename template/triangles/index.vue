<script>
  ...
  export default {
    methods: {
      start(element, width = window.innerWidth, height = window.innerHeight) {
        svg = this.create(element, width, height)
        setTimeout(() => this.animate(element), 1000)
      ...
      animate(element) {
        this.randomize();
        for(var i = 0; i < document.querySelector(`${element} .triangles`).childNodes.length; i++) {
            var polygon = document.querySelector(`${element} .triangles`).childNodes[i];
        ...
        setTimeout(() => this.animate(element), refreshDuration)
      }
    }
  }
</script>
