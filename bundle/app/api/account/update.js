...
const sendUser = account => Sices.sendUser({
  ...
  .then(status => ((status !== 304) ? 'Can not create User' : 'User and Account created successfully'))
...
const create = ({ object }) => Isquik.getAccount(object.id).then(isquikAccount => {
  const sicesUser = Sices.getUser(isquikAccount.owner)
  const sicesAccount = Sices.getAccount(sicesUser.owner)
})
