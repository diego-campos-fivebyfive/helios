...
class RangesController extends AbstractController
{
    ...
    public function putCostPriceRangeAction(Request $request, Range $range)
    {
        ...
        $precificatedPowerRanges = RangePrecify::calculate($range->getMetadata(), $costPrice);
        ...
        $formattedMarkupRanges = $rangeHelper->formatMarkup($powerRanges);

        return $this->json([
            'id' => $range->getId(),
            'costPrice' => $range->getCostPrice(),
            'powerRanges' => $formattedMarkupRanges,
    ...
    public function putCopyRangeAction(Request $request, Range $parentRange)
    {
        ...
        $result = $this->formatPowerRanges($childMetadata[$level]);

        return $this->json($result);
    }

    /**
     * @param array $powerRanges
     */
    private function formatPowerRanges(array $powerRanges)
    {
        foreach ($powerRanges as $key => $powerRange) {
            $powerRanges[$key]['markup'] *= 100;
        }
